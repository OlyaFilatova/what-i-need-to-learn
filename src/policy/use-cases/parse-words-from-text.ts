...
export class ParseWordsFromText {
  ...
  protected toSimpleText(text) {
    return text.replaceAll("wireMagic", ' ')
      .replaceAll("\"pens\"", ' ')
  ...
  public parse(text: string, knownWords: string[] = [], cleanRootDoubles = false): ParseResult {
    console.log('parsing ...')
    ...
    let knownAndUnknownWords
      = this.separateToKnownAndUnknown(
          uniqueWordsRes,
          knownWords
        );
  ...
  removeRootDoubles(wordCounter, uniqueWords) {
    uniqueWords.forEach((word) => {
      ...
      let possibleCopies = [
        ...immediateEndings.map(ending => word.slice(0, word.length - 1) + ending),
        ...immediateEndings.map(ending => word + ending)
      ];

      let hasIesEnding = word.slice(word.length - 3) === 'ies';
      let hasIedEnding = word.slice(word.length - 3) === 'ied';
      let hasSEnding = word.slice(word.length - 1) === 's';
      let hasEdEnding = word.slice(word.length - 2) === 'ed';
      ...
      for (let ending of immediateEndings) {
        if (word.endsWith(ending)) {
          alternativeWord = word.slice(0, word.length - ending.length);
        }
      }

      if (alternativeWord == undefined) {

        for (let possible of possibleCopies) {
          let count = this.removeWordWithEnding(wordCounter, uniqueWords, possible);
          if (count !== undefined) {
            addCount += count;
          }
        }
        ...
        let count = this.removeWordWithEnding(wordCounter, uniqueWords, word);
        if (count !== undefined) {
          addCount += count;
        }
  ...
  separateToKnownAndUnknown(uniqueWordsRes, knownWords) {
    ...
    existingKnownWords = uniqueWordsRes.filter(element =>
      knownWords.indexOf(element.getWord().getText()) !== -1);
    unknownWords = uniqueWordsRes.filter(element =>
      knownWords.indexOf(element.getWord().getText()) === -1);
